...
class _PhotosScreenState extends State<PhotosScreen> {
  ...
  String _formatSourceLastScan(int milliseconds) {
    if (milliseconds <= 0) return 'Not scanned yet';
      ...
      appBar: AppBar(
        ...
        actions: [
          if (_libraryPath != null) ...[
            ...
            IconButton(
              tooltip: 'People',
              onPressed: _selectionMode ? null : _openPeople,
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          'Photo Sources • ${_photoSources.length} connected',
                          style: Theme.of(context).textTheme.labelLarge
                              ?.copyWith(fontWeight: FontWeight.w900),
                        ),
                        const SizedBox(height: 8),
                        if (_photoSources.isEmpty)
                          const Text('No photo sources connected yet.')
                        else
                          Wrap(
                            spacing: 8,
                            runSpacing: 8,
                            children: _photoSources.map((source) {
                              ...
                              final lastScan =
                                  source['last_scan_milliseconds'] as int? ?? 0;
                              return Chip(
                                avatar: _PhotoSourceBrandIcon(
                                  sourceType: sourceType,
                                  displayName: name,
                                  size: 22,
                                ),
                                label: Text(
                                  '$name • $count photos • '
                                  '${_formatSourceLastScan(lastScan)}',
                                ),
                              );
                            }).toList(),
                          ),
                  ...
                  IconButton.filledTonal(
                    tooltip: 'Rescan all sources',
                    onPressed: _scanning || _photoSources.isEmpty
                        ? null
                        : _scanLibrary,
                    icon: const Icon(Icons.refresh),
                  ),
...
class _PhotoSourceBrandIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    switch (_resolvedType) {
      ...
      case 'onedrive':
        return Icon(Icons.cloud, size: size, color: const Color(0xFF0078D4));
      case 'icloud':
        return Icon(Icons.cloud, size: size, color: const Color(0xFF5AC8FA));
      default:
        return Icon(
