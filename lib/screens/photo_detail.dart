  ...
  Future<void> _writeToPhoto() async {
    ...
    try {
      ...
      if (result.success) {
        ...
        final descriptionVerified = expectedDescription.isEmpty ||
                      ...
                      child: ListView(
                        ...
                        children: [
                          ...
                          ExpansionTile(
                            ...
                            subtitle: const Text(
                              'Tags, private notes, and embedded file metadata',
                            ),
                            children: [
                              _ChipEditor(
                                label: 'Tags',
                          ...
                          FilledButton.icon(
                            ...
                            label: const Text('Save Photo Details'),
                          ),
                        ],
                      ),
                    ),
  ...
  Widget build(BuildContext context) {
    ...
    for (final name in photoNames) {
      ...
      if (familyId != null && familyById.containsKey(familyId)) {
        matchedAliasesByFamily.putIfAbsent(familyId, () => <String>[]).add(name);
      } else {
    ...
    for (final entry in familyById.entries) {
      ...
      final display = matched.isNotEmpty ? matched.first : entry.value.displayName;
      identityLabels.add(
        _PersonIdentityLabel(
          ...
          hasFace: matched.any(
            (alias) => faces.any(
              (face) =>
                  face.personName.trim().toLowerCase() ==
                  alias.toLowerCase(),
            ),
          ),
        ),
          ...
          children: [
            ...
            if (familyPeople.isNotEmpty) ...[
              ...
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: familyPeople.map((person) => Chip(
                  ...
                  child: Row(
                    children: [
                      CircleAvatar(
                        radius: 20,
                        backgroundImage:
                            thumbnail.existsSync() ? FileImage(thumbnail) : null,
